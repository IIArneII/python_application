...
def check_password(password):
    ...
    if len(password) < 8:
        raise LengthError('Длина пароля меньше 8 символов')
    if not re.findall('\d', password):
        raise DigitError('Пароль не содержит цифры')
    if not re.findall('[A-ZА-Я]', password) or not re.findall('[a-zа-я]', password):
        raise LetterError('Пароль должен содержать буквы обоих регистров')
    ...
    for i in pas:
        if not re.findall('[^a-zа-я]', i):
            if lit[i[1]] - lit[i[0]] == 1 and lit[i[2]] - lit[i[1]] == 1:
                raise SequenceError('Обнаружены тройка соседних на клавиатуре букв')
    return 'ok'
